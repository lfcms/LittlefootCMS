...
class tickets extends app
{
	...
	public function cat($vars)
	{
		...
		//include 'view/tickets.cat.php';
			
		include 'view/tickets.main.php';
	}
	/*
	public function closed($vars)
	{
		$this->main($vars, '', 'closed');
	}
	...
	public function backburner($vars)
	{
		$this->projecthome($vars, '', 'closed');
	}
	
	public function critical($vars)
	{
		$this->projecthome($vars, '', 'open');
	}*/
	...
	public function view($vars)
	{
		if(count($_POST))
		{
			...
			$filter = array('open', 'closed', 'backburner', 'critical');
		...
		$post = $this->db->fetch("
			SELECT b.*, u.user as user FROM hq_tickets b 
	...
	public function edit($vars)
	{
		...
		$row = $this->db->fetch("SELECT * FROM hq_tickets WHERE id = ".$id);
		
		$cats = $this->db->fetchall("SELECT DISTINCT category FROM hq_tickets WHERE project = '".mysql_real_escape_string($inst)."'");
	...
	public function create($vars)
	{
		if(count($_POST) > 0)
		{
			...
			$result = $this->db->query("
				INSERT INTO hq_tickets (`id`, `project`, `category`, `title`, `content`, `owner_id`, `date`, `status`)
				VALUES (
					NULL, 
		...
		redirect302($this->lf->appurl.$this->ini.'/tickets/view/'.$id.'/');
	}
	...
	public function newarticle($vars)
	{
		...
		$cats = $this->db->fetchall("SELECT DISTINCT category FROM hq_tickets WHERE project = ".intval($this->ini));
	...
	public function addcategory($vars)
	{
		if(count($_POST) > 0)
		{
			...
			redirect302($this->lf->appurl.$this->ini.'/tickets/view/'.$id.'/edit');
		}
