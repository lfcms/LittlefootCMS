<?php
	...
	if(!isset($_SESSION))
	{
		...
		$sess_name = dirname(__FILE__);
		$sess_name = str_replace(
			'system/lib/tinymce/jscripts/tiny_mce/plugins/ajaxfilemanager/inc', '', 
			$sess_name);
		$sess_name = preg_replace(
				'/[^a-zA-Z0-9]/', '', // Needs to be alphanumeric, remove other characters
				$sess_name.$_SERVER['SERVER_NAME']
		);
