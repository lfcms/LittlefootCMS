...
$like = '';
$options = '';
if($this->auth['user'] != 'anonymous' || true)
{
	//thread maker
	$sql = "
		SELECT
		...
		WHERE
			t.acl = 'public' OR t.owner_id = ".$this->auth['id']."
	";
	...
	else
	{
		...
		foreach($threads as $thread_id => $thread)
		{
			$output .= '
				<div id="thread_'.$thread_id.'" class="thread">
					...
					<div class="t_head">';
			if($thread['head']['owner_id'] == $this->auth['id'])
			{
				$protocol = 'http://';
				/*$url = array(
					$conf['domain'],
			...
			$output .= '
						<h4><a href="http://%baseurl%/wall/friends/'.$thread['head']['owner_id'].'">%disp'.$thread['head']['user'].'%</a> / <a href="http://%baseurl%/wall/'.$thread['head']['acl'].'">'.ucfirst($thread['head']['acl']).'</a>:</h4>
			...
			if(isset($thread['replies']))
			{
				...
				{	
					...
					$replace = array(
						"%form_action%" => 'http://%baseurl%/api.php?app=blog&cmd=rmpost',
						"%msg_id%" => $msg_id,
					...
					if($thread['head']['owner_id'] == $this->auth['id'] || $this->auth['id'] == $msg['sender_id'])
			...
			$output .=
					...
					'<form action="http://%baseurl%/api.php?app=blog&cmd=mkpost" method="post" class="add_post">
		...
		$sql = "SELECT id, display_name FROM lf_users WHERE id = ".$unique[0];
		unset($unique[0]);
		...
		//Like replace
		if(count($like) > 0 && $this->auth['user'] != 'anonymous')
		{
			$sql = "SELECT * FROM io_like WHERE user_id = ".$this->auth['id']." AND scope = 'int' AND (";
			...
			else
			{
				while($row = mysql_fetch_assoc($result))
				{
					...
					$with[] = '<a class="unlike hrefapi" href="%baseurl%'.$row['link'].'">Unlike</a>';
					unset($like[$row['link']]);
				}
				$output = str_replace($replace, $with, $output);
			}
			...
			{
				$replace[] = '%'.$var.'%'; 
				$with[] = '<a class="like hrefapi" href="%baseurl%'.$var.'">Like</a>';
			}
